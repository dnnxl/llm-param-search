...
N_BATCH = 10  # number of random search iterations
Q_BATCH = 2   # candidates per iteration
...
# INITIAL DATA
# =========================================================
train_x = torch.rand(Q_BATCH, bounds.shape[1], dtype=torch.double)
train_x_unnorm = unnormalize(train_x, bounds)
train_obj_true = evaluate_decoding_params(train_x_unnorm)
...
for iteration in range(N_BATCH):
    print(f"\n=== Random Search Iteration {iteration+1} ===")

    bo_start = time.time()

    # Random candidates
    new_x = torch.rand(Q_BATCH, bounds.shape[1], dtype=torch.double)
...
print("\nRandom search experiment completed successfully.")
