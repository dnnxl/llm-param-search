...
# USER CONFIG
# =========================================================
MODEL_NAME = "unsloth/Qwen3-0.6B-unsloth-bnb-4bit" #"unsloth/Llama-3.1-8B-Instruct-unsloth-bnb-4bit" #unsloth/Qwen3-0.6B-unsloth-bnb-4bit
DATA_PATH = "/home/danny.xie/data/dxie/llm-param-search/dataset/FEINA_test_split_train_2.csv"

DEVICE = "cuda" if torch.cuda.is_available() else "cpu"

N_BATCH = 10 # BO runs for 10 optimization rounds
MC_SAMPLES = 128 # Monte Carlo samples for acquisition function
REF_POINT = torch.tensor([20.0, 0.75], dtype=torch.double) # reference point for hypervolume (worse-than-expected SARI/BERT)
...
# =========================================================
print("Loading model...")
# Loads a 4-bit quantized Qwen or LLaMA model for fast inference.
...
# =========================================================
# Different models format outputs differently.
# Removes <think> reasoning tokens (Qwen)
# Extracts only assistant answer (LLaMA)
# Cleans extra spaces
# =========================================================
...
# =========================================================
# OBJECTIVE FUNCTION (NOW WITH INFERENCE TIME)
# =========================================================
def evaluate_decoding_params(x_unnorm: torch.Tensor):
    ...
    for row in x_unnorm:
        ...
        start_time = time.time()  # ⏱ inference timer start
        ...
        inference_time = time.time() - start_time  # ⏱ end timer
...
# Format: temperature, top_p, top_k, repetition_penalty, max_tokens
# BO works in normalized [0,1] space, then converts back.
bounds = torch.tensor([
    [0.1, 0.5,   10, 1.0,  20],
    [1.5, 1.0,  200, 2.0, 200],
], dtype=torch.double)
...
# =========================================================
# MAIN BO LOOP (NOW WITH BO TIMING)
# =========================================================
...
for iteration in range(N_BATCH):
    ...
    print(f"BO optimization time: {bo_time:.2f} sec")
